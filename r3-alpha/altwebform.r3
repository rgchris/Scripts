Rebol [
    ...
    Exports: [
        url-decode url-encode load-webform to-webform
    ]
...
url-decode: use [
    ...
    func [
        ...
        /wiki
        "Assumes `_` character is used to represent spaces"
    ][
        space: either wiki [#"_"] [#"+"]
...
url-encode: use [
    ...
    func [
        ...
        /wiki
        "Use `_` character to represent spaces"
    ][
        space: either wiki [#"_"] [#"+"]
        ...
        either parse text: to binary! text [
            copy text any [
                ...
                |
                [#"_" | #"."]
                (either wiki [percent-encode text] [text])
                |
                skip
                (percent-encode text)
            ]
...
load-webform: use [
    result path string pair as-path term
][
    result: copy []
    ...
    string: use [ch hx] [
        ...
        [
            ...
            ; any [unreserved | percent-encoded]
        ]
    ]
            ...
            (
                ...
                until [
                    ...
                    switch type?/word tree/1 [
                        none! [
                            if not tail? name [
                                insert/only tree tree: copy []
                            ]
                        ]

                        string! [
                            change/only tree tree: reduce [
                                tree/1
                            ]
                        ]
...
to-webform: use [
    ...
    emit: func [
        ...
        repend webform [
            "&" url-encode form-key "=" url-encode data
        ]
    ]
    ...
    value: [
        ...
        (emit to string! to word! here/1)
        |
        date!
        (emit replace mold here/1 "/" "T")
        |
        tag!
        (emit mold here/1)
        |
        ...
        any [
            ...
            (
                do make error! rejoin [
                    "Invalid value: "
                    copy/part mold any [reference here/1] 40
                ]
            )
        ]
    ...
    block: [
        ...
        and [
            ...
            (change/only here copy here/1)
            |
            [
                object! | map!
            ]
            (change/only here body-of here/1)
        ][
            into [
                object | array
            ]
    ...
    func [
        ...
        "Block or object to encode"

        /prefix
        "Includes the `?` character used to precede URL query strings"

        /ruby-style
        "Encodes structured keys using `a[b][c]` notation"
    ][
        clear path
        webform: copy ""
        data: either block? data [
            copy data
        ][
            body-of data
        ]
